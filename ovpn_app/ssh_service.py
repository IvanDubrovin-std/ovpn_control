...
class SSHCredentials:
    ...
    @classmethod
    def from_server(cls, server: "OpenVPNServer") -> "SSHCredentials":
        """
        Create SSH credentials from OpenVPNServer instance

        Args:
            server: OpenVPNServer model instance
...
class ISSHService(ABC):
    ...
    @abstractmethod
    async def execute_command(self, credentials: SSHCredentials, command: str) -> CommandResult:
    ...
    async def execute_command(self, command: str) -> CommandResult:
        ...
        try:
            ...
            stdout_str = (
                ...
                else (result.stdout.decode() if result.stdout else "")
            )
            stderr_str = (
                result.stderr
                if isinstance(result.stderr, str)
                else (result.stderr.decode() if result.stderr else "")
            )
...
class SSHService(ISSHService):
    ...
    async def download_file(self, credentials: SSHCredentials, remote_path: str) -> bytes:
        """
        ...
        Raises:
            SSHConnectionError: If download fails
        """
        connection = await self.create_connection(credentials)
        try:
            # Get the asyncssh connection object
            conn = connection._connection

            # Open SFTP session and read file content
            async with conn.start_sftp_client() as sftp:
                # Use open() to read file content directly into memory
...
class SSHCredentialsFactory:
    ...
    def from_server(server: OpenVPNServer) -> SSHCredentials:
        ...
        return SSHCredentials(
            ...
            private_key_content=(
                server.ssh_private_key if hasattr(server, "ssh_private_key") else None
            ),
        )
...
class OpenVPNCommandBuilder:
    ...
    @staticmethod
    def generate_server_config(port: int, protocol: str, subnet: str, netmask: str) -> str:
