...
class ServerForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            "ssh_private_key": forms.Textarea(
                attrs={
                    "rows": 6,
                    "class": "form-control",
                    "placeholder": "Вставьте SSH приватный ключ...",
                }
            ),
    ...
    def clean(self):
        ...
        if self.instance and self.instance.pk and self.instance.ssh_private_key:
            # Server has existing private key, no validation needed
            pass
        elif not ssh_password and not ssh_key_path:
...
class ClientCertificateForm(forms.ModelForm):
    ...
    def clean_name(self):
        ...
        if name and server:
            ...
            if (
                ClientCertificate.objects.filter(server=server, name=name)
...
class ServerConfigForm(forms.ModelForm):
    ...
    dns1 = forms.GenericIPAddressField(protocol="IPv4", label="DNS сервер 1", initial="8.8.8.8")
    dns2 = forms.GenericIPAddressField(protocol="IPv4", label="DNS сервер 2", initial="8.8.4.4")

    class Meta:
        model = OpenVPNServer
        fields = ["openvpn_port", "openvpn_protocol", "server_subnet", "server_netmask"]
...
class ServerSearchForm(forms.Form):
    ...
    search = forms.CharField(
        ...
        widget=forms.TextInput(
            attrs={"placeholder": "Поиск по имени, IP или описанию...", "class": "form-control"}
        ),
    )
...
class ClientSearchForm(forms.Form):
    ...
    search = forms.CharField(
        ...
        required=False,
        widget=forms.TextInput(
            attrs={"placeholder": "Поиск по имени или email...", "class": "form-control"}
        ),
    )
