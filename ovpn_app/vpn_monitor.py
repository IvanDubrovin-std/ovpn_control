...
from .models import VPNConnection, ClientCertificate, OpenVPNServer
...
class VPNMonitor:
    ...
    async def update_connections(self):
        ...
        if not active_connections:
            ...
            await sync_to_async(VPNConnection.objects.filter(
                client__server=self.server
            ).delete)()
            return
        ...
        for conn_data in active_connections:
            ...
            try:
                ...
                logger.info(f"Parsed connected_at: {connected_at} from '{conn_data['connected_since']}'")
            except Exception as e:
                logger.warning(f"Could not parse connected_since: {conn_data['connected_since']} - {e}")
        ...
        @sync_to_async
        def delete_inactive():
            return VPNConnection.objects.filter(
                client__server=self.server
            ).exclude(
                client__name__in=active_client_names
            ).delete()
