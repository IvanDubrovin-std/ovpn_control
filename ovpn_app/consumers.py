...
from .ssh_service import (
    SSHCredentialsFactory, SSHServiceContainer, 
    ISSHService, SSHConnectionError, SSHCommandError
)
...
class SSHTerminalHandler(IWebSocketMessageHandler):
    ...
    async def _handle_connect(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self._connection = await self._ssh_service.create_connection(credentials)
...
{self._server.ssh_username}@{self._server.host}:~$ """
    ...
    def _get_server(self) -> Optional[OpenVPNServer]:
        """Get server from database"""
        try:
            return OpenVPNServer.objects.get(id=self._server_id)
...
class BaseWebSocketConsumer(AsyncWebsocketConsumer):
    ...
    async def connect(self):
        ...
        # Authenticate user
        if not await self._authenticator.authenticate(self.scope):
    ...
    async def receive(self, text_data):
        ...
        try:
            ...
            await self.send(text_data=json.dumps({
                ...
                'message': 'Monitoring data request received'
            }))
