...
from .models import ClientCertificate, ServerTask
...
class OpenVPNService:
    ...
    def install_server(self, server, user):
        ...
        task = ServerTask.objects.create(
            server=server, task_type="install", task_id=str(uuid.uuid4()), created_by=user
        )
        ...
        try:
            ...
            result = self.ssh_service.execute_command("apt install -y openvpn easy-rsa", sudo=True)
            if result["exit_code"] != 0:
    ...
    def _generate_server_certificates(self, server):
        ...
        if not ca:
            # Build CA
            result = self.ssh_service.execute_command(
                'cd /etc/openvpn/easy-rsa && echo "yes" | ./easyrsa build-ca nopass', sudo=True
            )
            if result["exit_code"] != 0:
        ...
        result = self.ssh_service.execute_command(
            "cd /etc/openvpn/easy-rsa && ./easyrsa gen-dh", sudo=True
        )
        if result["exit_code"] != 0:
...
"""
        ...
        try:
            client = self.ssh_service.client
            if client is None:
    ...
    def revoke_client_certificate(self, client):
        ...
        try:
            ...
            # Generate CRL
            result = self.ssh_service.execute_command(
                "cd /etc/openvpn/easy-rsa && ./easyrsa gen-crl", sudo=True
            )
            if result["exit_code"] != 0:
