...
from .models import OpenVPNServer, ClientCertificate, VPNConnection, ServerTask
from .forms import ServerForm, ClientCertificateForm, ServerConfigForm
from .services import OpenVPNService, SSHService
...
class ServerDetailView(LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context.update({
            ...
            'active_connections': VPNConnection.objects.filter(
                client__server=server
            ).count(),
        })
...
class ClientCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        try:
            ...
            openvpn_service = OpenVPNService()
            client = openvpn_service.create_client_certificate(
                form.instance.server,
                form.instance.name,
                form.instance.email or ''
            )
            
            messages.success(
                self.request, 
                f'Сертификат для клиента "{form.instance.name}" успешно создан.'
            )
            return super().form_valid(form)
...
def revoke_client(request, client_id):
    ...
    try:
        ...
        return JsonResponse({
            'success': True,
            'message': f'Сертификат клиента "{client.name}" успешно отозван.'
        })
        
    except Exception as e:
        return JsonResponse({
            'success': False,
            'error': f'Ошибка при отзыве сертификата: {str(e)}'
        }, status=500)
...
def download_client_config(request, client_id):
    ...
    try:
        ...
        credentials = SSHCredentials(
            ...
            private_key_content=server.ssh_private_key if server.ssh_private_key else None
        )
        
        logger.info(f"SSH credentials prepared for {server.host}")
        
        # Create SSH service and client manager
        ssh_service = SSHService()
        client_manager = OpenVPNClientManager(ssh_service)
        
        # Download config from server
        success, filename, content = asyncio.run(
            client_manager.download_client_config(credentials, client.name)
        )
        
        logger.info(f"Download result: success={success}, filename={filename}, content_size={len(content) if content else 0}")
        
        if success and content:
            response = HttpResponse(content, content_type='application/x-openvpn-profile')
            response['Content-Disposition'] = f'attachment; filename="{filename}"'
            logger.info(f"Returning config file {filename}")
            return response
        else:
            error_msg = f'Не удалось скачать конфигурацию: файл не найден на сервере'
...
class ConnectionListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        return VPNConnection.objects.select_related(
            'client', 'client__server'
    ...
    def get_context_data(self, **kwargs):
        ...
        context.update({
            ...
            'servers_with_connections': connections.values(
                'client__server'
            ).distinct().count(),
            'total_servers': OpenVPNServer.objects.count(),
...
def monitoring_view(request):
    ...
    return render(request, 'ovpn_app/monitoring.html', {
        'servers': servers
    })
